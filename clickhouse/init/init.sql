...
AS analytics.daily_aggregates_local
...
AS analytics.session_facts_local
...
AS inventory.stock_movements_local
...
AS inventory.warehouse_snapshots_local
ENGINE = Distributed(cluster_2s1r, inventory, warehouse_snapshots_local, rand());

CREATE OR REPLACE FUNCTION factorial ON CLUSTER cluster_2s1r AS (n) -> toUInt64(arrayProduct(arrayMap(x -> toUInt64(x), range(1, toUInt64(n) + 1))));
